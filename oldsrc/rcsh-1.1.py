...
Railroad Command Shell
	
(C) Copyright 2000-2002 Dr. Peer Griebel  (peer.griebel@web.de)
Dieses Programmsystem unterliegt der GPL (siehe gpl.txt).

Erweiterungen der Version 1.1: Norbert Hohenbichler, Mai 2007

Die rcsh kommuniziert mit einem SRCP-konformen Server (z.B. erddcd
aus dem DDL-Projekt von Torsten Vogt.)

Fr die Untersttzung Danke an:
	Matthias Trute, mtrute@topmail.de
	Holger Wagner, hwagner@esg-gmbh.de
        Bernhard Schneider, Schneider-WF@t-online.de
        
Version 1.1
  Änderung auf SRCP Version 0.8.2, SRCP Version 7.x wird nicht mehr
  unterstützt, dabei gelten folgende Einschränkungen:
      - Rückmeldungen der Befehle (.... 100 OK ...) werden ignoriert
        (außer bei srcp.waitFB).
      - Zurückgebene Zeit wird ignoriert
      - nur getestet mit dem Server srcpd 2.0.10
      
  Öffnen mehrerer Kommando-Sitzungen nun möglich:
  Pro Aufruf von Connect() wird  eine neue Kommando-Sitzung mit dem
  SRCP-Server geöffnet. Den verschiedenen send-Befehlen
  (z.B. loco.send) kann dann mit dem Parameter socket_index
  festgelegt werden, welche Sitzung verwendet werden soll (Zählung
  startet bei 0). Vorteil: Auch in parallel laufenden Threads mit
  jeweils eigener Verbindung kann der WAIT - Befehl verwendet werden,
  welcher nur mehr die eigene Verbindung blockiert
    
  Manuelle Änderungen der Lok-Daten werden nicht mehr durch den
  Monitor-Thread überschrieben:
  Das loco-Objekt verfügt nun über ein Flag
  self.__has_unapplied_changes, welches gesetzt wird, wenn vom Bediener
  eine Lok-Eigenschaft verändert wird (setSpeed, setFunction, ...).
  Es wird rückgesetzt, wenn per send-Aufruf die Daten gesendet werden.
  Solange das Flag gesetzt ist, wird diese loco-Instanz nicht mehr durch
  den Monitor-Thread verändert. 
  Vorteil: Die Daten landen auch sicher bei der Lok und werden nicht
  mehr zwischenzeitlich mit dem bestehenden Lok-Status überschrieben.
    
  Neues Objekt hsignal:
  Dieses Objekt unterstützt Signale, welche mehrere GA-Ports benötigen,
  um alle Signalbilder darzustellen. Bis jetzt sind nur Lese-Methoden
  (getState) implementiert, um die vom SRCP-Projekt spdrs60 gestellten
  Signalbegriffe mit rcsh abfragen zu können.
    
  Kleinere Anpassungen an Python 2.4
     
        
Version 1.0
	Umstrukturierung von Methoden (die alten Methoden sind weiterhin
        als deprecated vorhanden)
        Einige Verbesserungen und Erweiterungen
        
Version 0.07
        Angepasst an Python 2.x
        Verwendung von eigenen Exceptions
        Verwaltung des Poll-Ports
        neue Methoden: getFB, 
	Accessories, Loks k�nen nun jeweils auf einen eigene
        srcp_connection zurck greifen.

Version 0.06
	Verbesserte Dokumentation
	Verwaltung des akt. Zustands in accessory

Version 0.04
	Fr das senden von \n ist nun die Klasse srcp_connection
		verantwortlich
	neue Fkt decodermaxspeed(); maxspeed() kann nun auch �dern
	Monitor: Auswertung Informationsport fr Loks
	private Variablen beginnen mit __
	Monitor(): Auswertung des Info-Kanals

Version 0.03
	erddcd ersetzt durch SRCP, da es auch andere Server gibt
	neue Klasse feedback
	neue Funktionen mhp0(), mhp1(), ..., nset(), nreset()
		als Abkrzung fur Signale, Weichen

Version 0.02
	Fehler beim senden von 0-Werten behoben
	kann auch ohne SRCP-Server betrieben werden
	Weitere Lok-Protokolle
	Zugriff auf Accessories
	
Version 0.01
	erste Ver�fentlichung
	
TODO:
	setf/togglef single-shot-mode
"""
...
#def DBG(s,f=open('rcsh.log','w')): f.write(s + '\n'); f.flush()
...
class srcp_connection:
	...
	"""
        ...
        """
        if __debug__: DBG('SRCP sending on command socket %d: %s' % 
                      (socket_index, s))
        ...
        if not self.connected(socket_index): raise Error, 'not connected'
    ...
    def setPower(self, p=1, bus=1, socket_index=0):
        ...
        if p: return self.sendget('SET %d POWER ON' % bus, socket_index)
        else: return self.sendget('SET %d POWER OFF' % bus, socket_index)
    ...
    def addLoco(self, l):
        ...
        if __debug__: DBG('storing loco [%s] with ident %s' % (l,locoident))
	...
	if address == None:
        ...
        if __debug__: DBG('storing accessory [%s] with ident %s' % (a,accident))
    ...
    def openInfoPort(self):
        ...
        """
        if __debug__: DBG('OpenInfoPort: %s %s' % (self.host,self.port))
        ...
        try:
            ...
            self.__InfoportS = None
            raise
        if __debug__: DBG('OpenInfoPort: %s' % self.__InfoportS)
    ...
    def addFBProc(self, portno, proc, bus=1):
        if not self.__fb_procs.has_key(bus):
            self.__fb_procs[bus] = {}
        if self.__fb_procs[bus].has_key(portno):
            self.__fb_procs[bus][portno].append(proc)
        else: self.__fb_procs[bus][portno] = [proc]	

    def clearFBProc(self, portno, bus=1):
        if self.__fb_procs.has_key(bus):
           if self.__fb_procs[bus].has_key(portno):
    ...
    def readDispatchInfo(self):
        ...
        for data in datalines:
            if __debug__: DBG('Monitor info: %s' % data)	    
	    ...
	    if w[2] != 'INFO': continue
            ...
            if w[4] == 'GL':
                locoident = w[5] + '/' + w[3] 
                if self.__locos.has_key(locoident):
                    self.__locos[locoident].parseinfo(data)
            elif w[4] == 'GA':
                accident = w[5] + '/' + w[3] 
                if self.__accessories.has_key(accident):
    ...
    def mainloop(self):
        ...
        if __debug__: DBG('Monitor: ports: ' + `selectports`)
...
class accessory:
    """
    General handling of an accessory of type NMRA or M�klin or PS
    """
...
class accessoryM(accessory):
    """
    Specialized accessory of type M�klin
    """
...
class parseinfo:
	...
	if i[1] == '101':		
	  ...
	  raise ProtocolError, "Answer code %s not expected." % i[1]
        ...
        if __debug__: DBG('parseinfo: %s' % self)
        self.valueChanged()
...
class loco(parseinfo):
    ...
    def setSpeed(self, s, parsemode=0):
        ...
        """
        if __debug__: DBG('addr %d, bus %d: setSpeed %s' 
    ...
    def getSpeed(self):
        """get speed setting"""
        if __debug__: DBG('addr %d, bus %d: getSpeed'
    ...
    def getFunction(self):
        ...
        if __debug__: DBG('%d: getFunction %s' % (self.__addr, self.__f[0]))
        return self.__f[0]

    def setFunction(self,v,parsemode=0):
        """set/get setting of FUNCTION"""
        if __debug__: DBG('addr %d, bus %d: setFunction %s->%s' %
    ...
    def toggleFunction(self,parsemode=0):
        ...
        if __debug__: DBG('addr %d, bus %d: toggleFunction %s'
    ...
    def setF(self, n, v,parsemode=0):
        """set Fn to v"""
        if __debug__: DBG('addr %d, bus %d: setF %d %s->%s' 
    ...
    def getF(self, n):
        """get value of Fn"""
        if __debug__: DBG('addr %d, bus %d: getF %s' 
    ...
    def toggleF(self, n, parsemode=0):
        """toggle value of Fn"""
        if __debug__: DBG('addr %d, bus %d: toggleF %d %s' 
    ...
    def getDirection(self):
        """get direction"""
        if __debug__: DBG('addr %d, bus %d: getDirection: %d' 
	     % (self.__addr, self.__bus, self.__dir))
        return self.__dir

    def setDirection(self, d, parsemode=0):
        """set direction"""
        if __debug__: DBG('addr %d, bus %d: setDirection %d->%s' %
